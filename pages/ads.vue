  ...
  <div>
    ...
    <section class="bg-blue-50 py-16 text-center border-b border-gray-200">
      <div class="container mx-auto px-4 max-w-5xl">
        <h1 class="text-4xl font-extrabold text-gray-900 mb-4">Jasoor Ads</h1>
        <p class="text-lg text-gray-700">Browse the latest opportunities and aid available in work, shelter, and psychological support.</p>
      </div>
    </section>
    ...
    <section class="py-10 bg-white">
      <div class="container mx-auto px-4 max-w-5xl">
        ...
        <div v-if="filteredAds.length === 0" class="text-center text-xl text-gray-600 py-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          >
            <h3 class="text-xl font-semibold text-blue-600 mb-3">{{ ad.title }}</h3>
            <p class="text-gray-700 mb-4 leading-relaxed">{{ ad.description }}</p>
            <p class="text-gray-600 text-sm mb-2"><strong>Category:</strong> {{ ad.category_display }}</p>
            <p v-if="ad.location" class="text-gray-600 text-sm mb-2"><strong>Location:</strong> {{ ad.location }}</p>
            <p v-if="ad.contact" class="text-gray-600 text-sm mb-4"><strong>Contact:</strong> {{ ad.contact }}</p>
            <NuxtLink :to="`/ads/${ad.id}`" class="inline-block px-6 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors duration-300">View Details</NuxtLink>
            <div class="card-shadow absolute top-0 left-0 w-full h-full pointer-events-none" :style="ad.shadowStyle"></div>
          </UCard>
        </div>
      </div>
    </section>
